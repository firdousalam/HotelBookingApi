...
router.post('/makeBooking', function (req, res) {
	...
	var add_on_services  = req.body.add_on_services;
	...
	var sql ="INSERT into booking (user_id, booking_on, booking_by,booking_by_admin_id,booking_from,booking_to,booking_rate,booking_amount,property_id) VALUES "+
	"("+user_id+", NOW(),'"+booking_by+"',"+booking_by_admin_id+",'"+booking_from+"','"+booking_to+"',"+booking_rate+","+booking_amount+",'"+property_id+"'); "
	global.db.query(sql, function (err, result) {
		if (err) throw err;
		var bookingId = result.insertId;
		for(var i=0;i<add_on_services.length;i++)
		{
			var charges = add_on_services[i];
			var sql ="INSERT into booking_add_on (booking_id, service_id, amount) VALUES "+
			"("+bookingId+","+charges["service_id"]+","+charges["amount"]+"); "
			global.db.query(sql, function (err, result) {
			});
		}
	});
...
router.post('/fetchBookingAll', function (req, res) {
	var sql = "select * from booking order by bookingId desc  limit 1000";
	global.db.query(sql, function (err, result) {
		if (err) throw err;
		console.log(result);
		res.send({error : 0,"message" : "credential match","data" : result});
...
router.post('/fetchBookingDetails', function (req, res) {
	var bookingId = req.body.bookingId;
	
	if(typeof bookingId == 'undefined' || bookingId == null)
	{
		res.send({error : 1,"message" : "Please provide booking id","data" : []});
		return;
	}
	var response = {};
	...
	global.db.query(sql, function (err, result) {
		...
		response.booking = result;
			var sql = "select aso.service_name,aso.room_type,bao.amount from booking_add_on bao,add_on_services aso where bao.booking_id ="+bookingId+" AND bao.service_id = aso.service_id";
			global.db.query(sql, function (err, result) {
				response.addOn = result;
			if (err) throw err;
			res.send({error : 0,"message" : "credential match","data" : response});
		});
	});
...
router.post('/updateBooking', function (req, res) {
	var bookingId = req.body.bookingId;
	var user_id = req.body.user_id;
	var booking_on = req.body.booking_on;
	var booking_by = req.body.booking_by;
	var property_id = req.body.property_id;
	var booking_by_admin_id = req.body.booking_by_admin_id;
	var add_on_services  = req.body.add_on_services;
	...
	var sql ="update booking SET user_id = "+user_id+", booking_on = '"+booking_on+"', booking_by = '"+booking_by+"',booking_by_admin_id="+booking_by_admin_id+",booking_from='"+booking_from+"',booking_to='"+booking_to+"',booking_rate="+booking_rate+",	booking_amount,property_id ='"+property_id+"' WHERE bookingId = "+bookingId;
	global.db.query(sql, function (err, result) {
		if (err) throw err;
		var sql ="delete  booking_add_on where booking_id ="+booking_id;
			global.db.query(sql, function (err, result) {
			});
		for(var i=0;i<add_on_services.length;i++)
		{
